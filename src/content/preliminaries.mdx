...
This naturally sets the stage for our twin results:
  ...
  2. For any EISS-stable $f^{\pi^\star}$,
     where $f$ may potentially be unstable, a simple modification to the data collection procedure using noise-injection (à la DART, @laskey2017dart) is sufficient to guarantee that $f^{\hat{\pi}}$ under the learned policy is EISS.
